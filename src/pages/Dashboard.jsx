...
function DashSection({ setSideOpen }) {
    ...
    const load = async () => {
      try {
        const results = await Promise.allSettled([
          getPengurus(), getAgenda(), getTausiyah(),
          getJadwalImam(), getPemasukan(), getPengeluaran()
        ])
        const val = (r) => r.status === 'fulfilled' ? r.value : []
        const [pD,aD,tD,jD,pmD,pgD] = results.map(val)
        setStats({
          pengurus:    pD.length,
          agenda:      aD.length,
          tausiyah:    tD.length,
          jadwal:      jD.length,
          pemasukan:   pmD.reduce((s,x)=>s+(Number(x.jumlah)||0), 0),
          pengeluaran: pgD.reduce((s,x)=>s+(Number(x.jumlah)||0), 0),
        })
      } catch(e) {
  ...
  return (
    <div>
      ...
      {loading && <p style={{color:'var(--text-muted)',marginBottom:'1rem'}}>⏳ Memuat data...</p>}
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:'1rem'}}>
        {cards.map(c=>(
          <div key={c.label} className="card" style={{padding:'1.5rem'}}>
            ...
            <p style={{fontSize:'1.3rem',fontWeight:700,color:c.color}}>{loading ? '...' : c.value}</p>
          </div>
        ))}
      </div>
    </div>
