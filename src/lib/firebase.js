...
export const saveJadwalImam = async (data) => {
  if (data.id) { const { id, ...rest } = data; await setDoc(doc(db, 'jadwal_imam', id), rest, { merge: true }); return id }
  const ref = await addDoc(collection(db, 'jadwal_imam'), data); return ref.id
}
